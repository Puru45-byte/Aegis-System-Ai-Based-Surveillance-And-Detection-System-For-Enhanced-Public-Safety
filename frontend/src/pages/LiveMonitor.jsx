...
import liveDetectionSocket from '../services/liveDetectionSocket';
...
const CameraModal = ({ cameraData, onClose, onSave }) => {
    ...
    const [form, setForm] = useState(cam || { name: '', location: '', rtsp_url: '', is_active: true, latitude: null, longitude: null });
    const [saving, setSaving] = useState(false);
    const [latitude, setLatitude] = useState(cam?.latitude || null);
    const [longitude, setLongitude] = useState(cam?.longitude || null);
    ...
    return (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
            <div className="bg-[#1a1a1a] border border-borderContent rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                <div className="flex items-center justify-between p-4 border-b border-borderContent flex-shrink-0">
                ...
                <div className="p-4 space-y-4 overflow-y-auto flex-1">
                    ...
                    {/* Location Picker Map - Reduced height */}
                    <div>
                        <label className="text-xs text-textMuted mb-1 block">Camera Location (Click on map to set)</label>
                        <div className="h-[200px] w-full rounded-xl overflow-hidden border border-borderContent">
                            <MapContainer 
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label className="text-xs text-textMuted mb-1 block">Latitude</label>
                            <input 
                                value={latitude || ''} 
                                readOnly
                                className="w-full bg-[#18181b] border border-borderContent rounded-xl py-2.5 px-4 text-textMuted outline-none text-sm font-mono"
                                placeholder="Click map to set" 
                            />
                        </div>
                        <div>
                            <label className="text-xs text-textMuted mb-1 block">Longitude</label>
                            <input 
                                value={longitude || ''} 
                                readOnly
                                className="w-full bg-[#18181b] border border-borderContent rounded-xl py-2.5 px-4 text-textMuted outline-none text-sm font-mono"
                ...
                <div className="flex items-center gap-3 p-4 border-t border-borderContent flex-shrink-0">
...
const CameraFeed = ({ cam, onEdit, onDelete, featured = false }) => {
    ...
    const captureFrame = () => {
        ...
        try {
            ...
            // Non-blocking API call with authentication
            axiosClient.post('/detection/live-scan', {
                image: imageData,
                camera_id: cam.id,
            ...
            .catch(err => {
                console.error("❌ Live scan failed:", err.response?.data || err.message);
                setFaceDetecting(false);
                ...
                if (err.response?.status === 401) {
                    ...
                    console.error("Invalid request format - check camera data");
    ...
    // Safety check: if camera doesn't exist, show error
    if (!cam) {
        return (
            <div className="bg-[#0c0c0f] border border-borderContent rounded-2xl overflow-hidden flex flex-col">
                <div className="px-4 py-3 border-b border-borderContent bg-[#111114]">
                    <p className="font-semibold text-textMain text-sm">No Camera Data</p>
                </div>
                <div className="flex-1 flex items-center justify-center">
                    <CameraOff className="w-8 h-8 mx-auto text-zinc-700 mb-2" />
                    <p className="text-xs text-zinc-600">Camera not available</p>
                </div>
            </div>
        );
    }
...
const LiveMonitor = () => {
    ...
    useEffect(() => {
        ...
        if (token) {
            ...
            liveDetectionSocket.onStatusChange((status) => {
                console.log('🔄 Detection status changed:', status);
                setIsDetectionActive(status.active);
            });

            liveDetectionSocket.onConnect(() => {
                console.log('✅ WebSocket connected');
                setSocketConnected(true);
            });

            liveDetectionSocket.onDisconnect(() => {
                console.log('🔌 WebSocket disconnected');
                setSocketConnected(false);
                setIsDetectionActive(false);
            });

            liveDetectionSocket.onError((error) => {
                console.error('❌ WebSocket error:', error);
                setSocketConnected(false);
            });

            // Connect WebSocket
            liveDetectionSocket.connect(token);

            // Start detection when component mounts
            setTimeout(() => {
                if (liveDetectionSocket.isSocketConnected()) {
                    liveDetectionSocket.startDetection();
                }
            }, 1000);
        }
        ...
        return () => {
            if (token) {
                liveDetectionSocket.stopDetection();
                liveDetectionSocket.disconnect();
            }
        };
    }, []);
